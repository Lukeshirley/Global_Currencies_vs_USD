...
def calculate_performance(df):
    ...
    # Format the performance values to x.xx %
    performance.iloc[:, 1:] = performance.iloc[:, 1:].applymap(lambda x: f"{x:.2f} %")
    return performance
...
def main():
    ...
    print(performance.head())  # Debugging: Check the performance DataFrame
